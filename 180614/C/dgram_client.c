...
int main(int argc, char** argv) {
    ...
    while(scanf("%s", nomeFile) == 1) {
        strcpy(req.fileName, nomeFile);

        printf("Invio richiesta di eliminazione delle consonanti nel file %s.\n", req.fileName);
        ...
        if(ris < 0) {
            printf("Cliente: file inesistente o errore in apertura\n");
        } else {
            printf("Cliente: numero di consonanti eliminate = %d\n", ris);
        }
        printf("Dammi il nome di file, EOF per terminare: ");
    }  //while
}  //main